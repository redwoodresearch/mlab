...
class UniModule(t.nn.Module):
    ...
    def forward(self, x, past_key_values=None, return_key_values=False): # b seq_len hidden_size
        if past_key_values is None:
            ...
            print("OLD QK", return_key_values)
            print(qk)
            ...
            if return_key_values:
                ...
                print("K V COMPUTED FROM SCRATCH:")
                print(k)
                print(v)
                print("\n\n")
            ...
            qk /= self.head_size**0.5
            print("OLD QK", return_key_values, qk)
            qk = F.softmax(qk, dim=-1)
            print("NEW QK", qk)
            v = t.cat((v, v_x), dim=-2)
            combined = t.einsum('bnij,bnjh->bnih', qk, v)
...
class GPT2Module(t.nn.Module):
    def __init__(self, 
        ...
        self.register_buffer('cache', t.zeros((num_layers, num_heads, 0, 2 * self.head_size)))
