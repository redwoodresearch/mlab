...
def get_minigpt(fname):
    ...
    out["token_embedding.weight"] = weights["embedding.token_embedding.weight"]
    out["pos_embedding.weight"] = weights["embedding.position_embedding.weight"]
    for i in [0, 1]:
        out[f"blocks.{i}.project_qkv.weight"] = weights[
            f"blocks.{i}.attention.attention_weights.weight"
        ]
        out[f"blocks.{i}.project_output.weight"] = weights[
            f"blocks.{i}.attention.project_output.weight"
        ]
