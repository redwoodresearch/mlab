...
def raw_attention_pattern(
    ...
    Q = rearrange(
        Q, "b seqlen (headnum headsize) -> b headnum seqlen headsize", headnum=num_heads
    )
    K = project_key(token_activations)
    K = rearrange(
        K, "b seqlen (headnum headsize) -> b headnum seqlen headsize", headnum=num_heads
    )
    headsize = K.shape[-1]
...
class LayerNorm(nn.Module):
    ...
    def forward(self, input):
        ...
        input_m0v1 = (
            input_m0 / input_m0.std(dim=-1, keepdim=True, unbiased=False).detach()
        )
