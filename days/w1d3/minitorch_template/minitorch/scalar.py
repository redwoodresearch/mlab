...
from .autodiff import Context, FunctionBase, History, Variable

# ## Task 1.1
# Central Difference calculation


def central_difference(f, *vals, arg: int = 0, epsilon: float = 1e-6):
    r"""
    ...
    """
    # (f(x0, x1 + eps / 2, x2) - f(x0, x1 - eps / 2, x2)) / eps

    vals_low = list(deepcopy(vals))
    vals_low[arg] -= epsilon / 2

    vals_high = list(deepcopy(vals))
    vals_high[arg] += epsilon / 2

    return (f(*vals_high) - f(*vals_low)) / epsilon
