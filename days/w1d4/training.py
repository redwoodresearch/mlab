    ...
    def step(self):
        with torch.no_grad():
            for i, param in enumerate(self.params):
                ...
                param.data = param.data - self.lr * mt_hat / (
                    torch.sqrt(vt_hat) + self.eps
                )
...
def train(
    ...
    print(locals())
    l = locals()
    opt_args = {i: l[i] for i in opt_data[1:]}
...
experiment = Experiment(
    api_key="Mg7SiAgXBTuftVI8WEDNmieKp",
...
with gin.unlock_config():
    gin.parse_config_files_and_bindings(
        [get_fullpath("config.gin")], params["GIN_CONFIG"]
    )
