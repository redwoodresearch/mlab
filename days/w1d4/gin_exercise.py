...
if __name__ == "__main__":
    ...
    possible_values = {"Model.hidden_size" : [256, 512, 1024],
                        "Model.num_hidden_layers": [1, 2, 3],
                    }
    ...
    for hyperparams_dict in grid:
        ...
        experiment = Experiment(
            ...
            project_name="mlab retry",
            workspace="msimontaylor",
        )
        experiment.log_parameters(hyperparams_dict)

        hyperparams = []

        # Formatting
        for key, val in hyperparams_dict.items():
            hyperparams.append(f"{key}={val}")
        
        with gin.unlock_config():
            gin.parse_config_files_and_bindings(["config.gin"], hyperparams)
